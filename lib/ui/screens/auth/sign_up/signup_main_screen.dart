...
import 'package:Ziepick/ui/screens/widgets/custom_text_form_field.dart';
...
import 'package:Ziepick/utils/login/lib/login_status.dart';
import 'package:Ziepick/utils/login/lib/payloads.dart';
...
class LoginScreenState extends State<SignUpMainScreen> {
  ...
  final TextEditingController referTextController = TextEditingController();
  ...
  late PhoneLoginPayload phoneLoginPayload =
      PhoneLoginPayload(emailMobileTextController.text,referTextController.text.trim(), countryCode!);
  bool isBack = false;
  ...
  void _onTapContinue() {
    if (isMobileNumberField) {
      Navigator.pushNamed(context, Routes.mobileSignUp, arguments: {
        ...
        "refer": referTextController.text.toString().trim(),
  ...
  Widget mobileAndEmailSignUp() {
    return Column(
      ...
      children: [
        CustomText(
          "signUpToZiepick".translate(context),
        ...
        SizedBox(height: 10,),
        CustomTextFormField(
            controller: referTextController,
            fillColor: context.color.secondaryColor,
            borderColor: context.color.textLightColor.withValues(alpha: 0.2),
            // onChange: (value) {
            //   //bool isNumber = value.toString().contains(RegExp(r'^[0-9]+$'));
